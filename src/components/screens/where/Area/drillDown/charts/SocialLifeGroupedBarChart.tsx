...
export const SocialLifeGroupedBarChart = () => {
  ...
  const dataForBarChart = useMemo(() => {
    if (drillDownData?.getDrillDowns) {
      return transformDataForBarChart(
        drillDownData?.getDrillDowns as GetDrillDowns_getDrillDowns_DrillDownSocialLifeLocation[],
...
const transformDataForBarChart = (
  data: GetDrillDowns_getDrillDowns_DrillDownSocialLifeLocation[],
    ...
    .reduce<DataForBarChart>((accumulator, regionItem) => {
      ...
      const list: DataForBarChart =
        regionItem?.data?.map((item) => {
          const group = {
            id: item.socialFactor.toLowerCase().replace(/\s/g, '-'),
            label: item.socialFactor,
            color: item.color ?? '',
            isSelected: item.selectedByUser,
          }
          return {
            id: category.id + group.id,
            value: roundValue(item.percentAgeCategory),
            category,
            group,
          }
        }) ?? []
